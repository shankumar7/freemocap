"""freemocap.live_capture

Packaged live capture PoC using MediaPipe.
Run with:
    python -m freemocap.live_capture --camera 0 --width 1280 --height 720 --backend holistic

This module mirrors the experimental PoC and is safe to import; it will only
access the camera when run as a script or when `run()` is called.
"""
...
def run(camera_index: int = 0, width: int = 640, height: int = 480, backend: str = "holistic") -> int:
    """Start live capture and display landmarks.

    Returns 0 on clean exit or 1 on camera open failure.
    """
    ...
    try:
        while True:
            ...
            cv2.imshow("FreeMoCap Live", image)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(prog="freemocap.live_capture")
